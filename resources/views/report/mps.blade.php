    ...
    <div class="container-fluid">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active"><a href="#">MPS</a></li>
            </ol>
        </nav>
    </div>
</nav>
...
<div class="d-flex justify-content-between align-items-end flex-wrap gap-3 mb-3">
    ...
    <form method="GET" action="{{ route('reports.mps') }}" class="d-flex  gap-2 ">
        <!-- Start Date -->
...
<h3>Locked Up and Visited Graph</h3>
...
<h3>Current Locked Up Students </h3><br>
<table class="table table-responsive table-sm mb-4">
    ...
    <tbody>
        ...
        $i = 0;
        @endphp
        @foreach ($currentLockedUpStudents as $student)
...
<td><a href="{{route('mps.show', $student['student']->id)}}" class="btn btn-sm btn-info">View</a></td>
    </tr>
        @endforeach
...
<h3>Most Locked Up Students </h3><br>
<table class="table table-responsive table-sm">
    ...
    <tbody>
        ...
        $i = 0;
        @endphp
        @foreach ($topLockedUpStudents as $student)
        <tr class="txt-danger" style="color:red">
            <td style="{{ $student['count'] > 2 ? 'color:red' : '' }}">{{++$i}}.</td>
            <td style="{{ $student['count'] > 2 ? 'color:red' : '' }}">{{ $student['student']->force_number?? '' }} {{ $student['student']->first_name }} {{ $student['student']->last_name }}</td>
            <td style="{{ $student['count'] > 2 ? 'color:red' : '' }}">{{ $student['student']->company->name }} - {{ $student['student']->platoon }}</td>
...
<h3>Most Visited Students </h3><br>
<table class="table table-responsive table-sm">
    <thead>
        <tr>
            <th>S/N</th>
    ...
    <tbody>
        ...
        $i = 0;
        @endphp
        @foreach ($topVisitedStudents as $student)
        <tr class="txt-danger" style="color:red">
            <td style="{{ $student['count'] > 2 ? 'color:red' : '' }}">{{++$i}}.</td>
            <td style="{{ $student['count'] > 2 ? 'color:red' : '' }}">{{ $student['student']->force_number?? '' }} {{ $student['student']->first_name }} {{ $student['student']->last_name }}</td>
            <td style="{{ $student['count'] > 2 ? 'color:red' : '' }}">{{ $student['student']->company->name }} - {{ $student['student']->platoon }}</td>
            <td style="{{ $student['count'] > 2 ? 'color:red' : '' }}">{{ $student['count']}}</td>
<td><a href="{{route('visitors.show', $student['student']->id)}}" class="btn btn-sm btn-info">View</a></td>
    </tr>
        @endforeach
...
const dailyData = {
    ...
    datasets: [{
        ...
        {
            label: 'Visisted Trends',
            data: dailyCounts.visitor_counts,
            type: 'line',
            fill: false,
            borderColor: 'rgba(161, 190, 28, 0.63)',
            tension: 0.1,
            hidden: true
        },

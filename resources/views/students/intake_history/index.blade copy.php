  ...
  <div class="card-body" style="margin-right: 0px;">
    <div class="row" style="margin-right: -10px;">
      ...
      <div class="col-md-3">
        ...
        <button class="card bg-{{ $type['color'] }} text-white w-100 filter-card" data-type="{{ $type['key'] }}">
          <div class="card-body text-center">
            <h5>{{ $type['label'] }}</h5>
            <p class="fs-4">{{ $stats[$type['key']]->count() ?? 0 }}</p>
          </div>
...
<script>
  ...
  function showStudents(type, page = 1) {
    ...
    for (const [key, value] of formData.entries()) {
      if (key.endsWith('[]')) {
        params.append(key, value); // Already correct
      } else {
        params.append(key, value); // Use append for all to preserve duplicates
      }
    }

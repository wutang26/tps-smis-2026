...
<a class="btn btn-success mb-2"
   href="{{ route('patrol-areas.create') }}"
   style="float:right !important; margin-right:1%">
   <i class="fa fa-plus"></i> Create New Patrol Area
</a>
</div>
...
<form id="deleteForm{{ $patrolArea->id }}"
      action="{{ route('patrol-areas.destroy', $patrolArea) }}"
      method="POST"
      style="display:inline;">
@csrf
@method('DELETE')
<button class="btn btn-sm btn-danger"
        onclick="confirmDelete('deleteForm{{ $patrolArea->id }}','Patrol Area')"
        type="button">
Delete
</button>
</form>
...
@if(isset($patrolArea->beat_exceptions) && $patrolArea->beat_exceptions->isNotEmpty())
<ol>
@foreach ($patrolArea->beat_exceptions as $beat_exception)
<li>{{ $beat_exception->name }}</li>
@endforeach
...
{{-- Beat Time Exceptions --}}
<div class="mb-4 mt-4">
<h3>Beat Time Exceptions</h3>
</div>

@if(isset($patrolArea->beat_time_exceptions) && $patrolArea->beat_time_exceptions->isNotEmpty())
<ol>
@foreach ($patrolArea->beat_time_exceptions as $beat_time_exception)
<li>{{ $beat_time_exception->name }}</li>
@endforeach
...
<div class="modal-footer">
<a class="btn btn-sm btn-primary"
   href="{{ route('patrol-areas.edit', $patrolArea) }}">
Edit
</a>
</div>

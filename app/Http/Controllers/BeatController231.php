    ...
    public function generatePDF(Request $request, $companyId)
{
    ...
    foreach ($areas as $areaData) {
        ...
        /** ----------------------------------------
 * GENDER / EXCEPTION LOGIC (STRICT NIGHT FOR MALES)
 * ---------------------------------------- */


// $requiredSlots = $area->required_slots ?? 4;
...
// } else {
//     if ($startAt === '06:00' || $startAt === '12:00') {
//         // Day shift → females first
//         $females = $companyStudents->where('gender', 'F');
//         $males = $companyStudents->where('gender', 'M');

//         $companyStudents = $females;

//         // Fill remaining slots with males based on last 4 days
//         $remainingSlots = $requiredSlots - $females->count();
//         if ($remainingSlots > 0) {
//             $startDate = now()->subDays(4)->startOfDay();
//             $endDate = now()->subDays(1)->endOfDay();

//             // Males assigned in last 4 days, most recently first
//             $recentMaleIds = Assignment::whereIn('student_id', $males->pluck('id'))
//                 ->whereBetween('date', [$startDate, $endDate])
//                 ->orderBy('date', 'desc')
//                 ->pluck('student_id')
//                 ->toArray();

//             $recentMales = $males->whereIn('id', $recentMaleIds);
//             $fillCount = min($remainingSlots, $recentMales->count());
//             $companyStudents = $companyStudents->merge($recentMales->take($fillCount))->values();

//             // Fill remaining slots with other males not assigned in last 4 days
//             $remainingSlots = $requiredSlots - $companyStudents->count();
//             if ($remainingSlots > 0) {
//                 $otherMales = $males->whereNotIn('id', $recentMaleIds);
//                 $companyStudents = $companyStudents->merge($otherMales->take($remainingSlots))->values();
//             }
//         }
//     } else {
//         // Night shift → males only
//         $companyStudents = $companyStudents->where('gender', 'M')->values();
//     }
// }

// // Final fallback: fill any remaining slots with placeholder
// $remainingSlots = $requiredSlots - $companyStudents->count();
// if ($remainingSlots > 0) {
...
//         ]);
//     }
//     $companyStudents = $companyStudents->merge($placeholders)->values();
// }

//-------------------YA PILI----------------//

/** ----------------------------------------
 * GENDER / EXCEPTION LOGIC (COOLDOWN SAFE)
 * ---------------------------------------- */
if (!empty($area->beat_exception_ids)) {
    ...
    if ($startAt === '06:00' || $startAt === '12:00') {
        // Day shift → allow BOTH genders (no priority, no reorder)
        $companyStudents = $companyStudents->values();
...
// $sortByRest = function ($students) use ($today) {
//     return $students->sortByDesc(function ($student) use ($today) {

//         if (!$student->last_assigned_at) return 999;

//         $date = $student->last_assigned_at instanceof \Carbon\Carbon
//             ? $student->last_assigned_at
//             : \Carbon\Carbon::parse($student->last_assigned_at);
...
//     }
// } else {
//     // No exceptions, apply default gender rules
//     if ($startAt === '06:00' || $startAt === '12:00') {
//         // Day shifts → prefer females, fallback to males if not enough
//         $preferred = $companyStudents->where('gender', 'F');

//         if ($preferred->isNotEmpty()) {
//             $companyStudents = $preferred->merge($companyStudents->diff($preferred))->values();
//         } else {
//             $companyStudents = $companyStudents->values();
//         }
//     } else {
//         // Night shifts → males only
//         $companyStudents = $companyStudents->where('gender', 'M')->values();
//     }
// }

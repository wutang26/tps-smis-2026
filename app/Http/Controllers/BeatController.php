...
class BeatController extends Controller
{
    ...
    private function updateSummary(&$summary, &$totalPlatoonCount, $beat)
    {
        ...
        if (is_string($beat->student_ids)) {
    $studentIds = json_decode($beat->student_ids, true) ?? [];
} elseif (is_array($beat->student_ids)) {
    $studentIds = $beat->student_ids;
} else {
    $studentIds = [];
}
    ...
    //     $cooldownDate = Carbon::parse($date)->subDays(3); //rest period of 3 days
    ...
    //         /** ----------------------------------------
    //          * BASE ELIGIBLE STUDENTS (WITH 3-DAY REST)
    //          * ---------------------------------------- */
    ...
    //         // --------------------------------------
    //         // FINAL FALLBACK: never leave beat empty
    //         if (empty($eligible)) {
    ...
    public function generateBeats($areas, $studentsByCompany, $studentsByPlatoon, $beatType, $date, &$usedStudentIds)
{
    ...
    foreach ($areas as $areaData) {
        ...
        foreach ($companyStudents as $s) {
            ...
            // enforce current group
            if (!in_array($s->platoon, $currentGroup)) continue;

            if (isset($usedMap[$s->id])) continue;

            // ignore 3-day cooldown (we allow all)
            // if ($s->last_assigned_at && \Carbon\Carbon::parse($s->last_assigned_at)->gt(now()->subDays(3))) continue;

            // night shift: females cannot work
            if ($isNightShift && $s->gender === 'F') continue;
        ...
        // -------------------------
        // Fasting restriction: cannot work 12:00–23:59
        if ($hour >= 12 && $hour < 24) {
            $eligible = array_filter($eligible, fn($s) => $s->fast_status != 1);
        ...
        if (!empty($area->beat_exception_ids)) {
            ...
            $preferred = array_filter($eligible, fn($s) => $s->gender === $preferredGender);
            if (!empty($preferred)) {
                $eligible = array_values($preferred);
            }
        }

        // -------------------------
        // FINAL FALLBACK: if still empty, pick anyone from currentGroup
        if (empty($eligible)) {
            $eligible = $companyStudents
                ->filter(fn($s) => in_array($s->platoon, $currentGroup))
                ->values()
        ...
        // -------------------------
        // Sort by last area + beat_round + id
        $currentAreaId = $area->id;
        usort($eligible, function ($a, $b) use ($lastAreaMap, $currentAreaId) {
            $aRepeat = ($lastAreaMap[$a->id] ?? null) === $currentAreaId;
            $bRepeat = ($lastAreaMap[$b->id] ?? null) === $currentAreaId;
...
//End of New One For Muslim
    ...
    // {
    //     // Determine cooldown date (3 days)
    //     $cooldownDate = Carbon::parse($date)->subDays(3);
    ...
    //         ->where('company_id', $companyId)
    //         ->where('session_programme_id', 10)
    //         ->where('fast_status', 0) //Pick only non-fasting students for reserves (since they may be called last-minute)
    ...
    //     $reserves = $reserves
    //     ->merge($additionalReserves)
    //     ->take(10) //  HARD LIMIT GUARANTEE Since we only need 10 reserves
    //     ->map(function ($student) use ($companyId, $date, &$usedStudentIds) {
    ...
    //         'company_id' => $companyId,
    //         'beat_date' => $date,
    //         'beat_round' =>BeatRound::where('company_id', $companyId)
    //                      ->value('current_round') ?? 0,
    //         'created_at' => now(),
    //         'updated_at' => now(),
    //     ];
    // })
    ...
    //     // Insert reserves into the database
    //         if (!empty($reserves)) {
    //     BeatReserve::insert($reserves);
    // }
    ...
    public function assignReserves($companyId, $date, &$usedStudentIds)
{
    ...
    // Check if reserves already exist for this date
    $existingReserves = BeatReserve::where('beat_date', $date)
        ->where('company_id', $companyId)
        ->exists();
    ...
    // Already assigned student IDs (Guard, Patrol, Leaders)
    $assignedStudentIds = Beat::where('date', $date)
        ->where(function ($q) use ($companyId) {
            $q->whereHas('guardArea', fn($q) => $q->where('company_id', $companyId))
              ->orWhereHas('patrolArea', fn($q) => $q->where('company_id', $companyId));
        ...
        ->pluck('student_ids')
        ->map(fn($ids) => is_array($ids) ? $ids : json_decode($ids, true))
        ->flatten()
        ->toArray();
    ...
    // Fetch eligible students: beat_status=1, fast_status=0, not already assigned, cooldown passed
    $eligibleStudents = Student::where('beat_status', 1)
        ->where('fast_status', 0) // <-- only fasting=0
        ->where('company_id', $companyId)
        ->where('session_programme_id', 10)
        ->whereNotIn('id', $alreadyAssigned)
        ->where(function ($q) use ($cooldownDate) {
            $q->whereNull('last_assigned_at')
              ->orWhere('last_assigned_at', '<=', $cooldownDate);
        })
        ->get()
        ->shuffle() // make selection fair
        ->values();
    ...
    $studentsByPlatoon = $eligibleStudents->whereIn('platoon', $currentGroup)->groupBy('platoon');
    ...
    // Pick one male and one female per platoon until quotas met
    foreach ($platoonsInGroup as $platoon) {
        if ($reserves->count() >= 10) break;

        $platoonStudents = $studentsByPlatoon[$platoon] ?? collect();

        // Male
        if ($reserves->where('gender', 'M')->count() < $totalMaleNeeded) {
            $male = $platoonStudents->where('gender', 'M')
                    ->whereNotIn('id', $reserves->pluck('id'))
                    ->first();
            if ($male) $reserves->push($male);
        }

        // Female
        if ($reserves->where('gender', 'F')->count() < $totalFemaleNeeded) {
            $female = $platoonStudents->where('gender', 'F')
                      ->whereNotIn('id', $reserves->pluck('id'))
                      ->first();
            if ($female) $reserves->push($female);
        }
    }

    // Fill remaining quotas if not enough from platoons
    $maleRemaining = $totalMaleNeeded - $reserves->where('gender', 'M')->count();
    $femaleRemaining = $totalFemaleNeeded - $reserves->where('gender', 'F')->count();
    $remainingSlots = 10 - $reserves->count();

    if ($remainingSlots > 0) {
        $additionalMales = $eligibleStudents->where('gender', 'M')
            ->whereNotIn('id', $reserves->pluck('id'))
            ->take($maleRemaining);
        $additionalFemales = $eligibleStudents->where('gender', 'F')
            ->whereNotIn('id', $reserves->pluck('id'))
            ->take($femaleRemaining);

        $reserves = $reserves->merge($additionalMales)
                             ->merge($additionalFemales)
                             ->take(10) // hard limit
                             ->values();
    }

    // Prepare for insertion
    $reservesArray = $reserves->map(function ($student) use ($companyId, $date, &$usedStudentIds) {
        $usedStudentIds[] = $student->id;
        return [
            'student_id' => $student->id,
            'company_id' => $companyId,
            'beat_date'  => $date,
            'beat_round' => BeatRound::where('company_id', $companyId)->first()->current_round ?? 0,
        ];
    })->toArray();

    if (!empty($reservesArray)) {
        BeatReserve::insert($reservesArray);
    }

    // Update beat_status for selected reserves
    Student::whereIn('id', collect($reservesArray)->pluck('student_id'))->update(['beat_status' => 2]);

    return $reservesArray;
}

    //-------------Reserve ASSIGNMENT MODIFIED-----------------
